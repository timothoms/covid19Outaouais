...
```{r rls_new, eval = FALSE}
keys <- list(
  list(c("Total", "Total cases", "Total cases (RLS)", "RLS de Gatineau"), title = "Regional total & Gatineau"),
  list(c("RLS de la Vallée-de-la-Gatineau", "RLS de Papineau"), title = "Papineau & Vallée-de-la-Gatineau"),
  list(c("RLS des Collines-de-l'Outaouais", "RLS du Pontiac"), title = "Collines & Pontiac")
)
...
```
<!-- ### Active cases -->
```{r rls_active, eval = FALSE}
lapply(keys, function(selection) {
  ggplot(data = covid[covid$key %in% selection[[1]] & covid$table == "rls_active", ]) +
...
keys <- c("RLS de Gatineau", "RLS de la Vallée-de-la-Gatineau", "RLS de Papineau", "RLS des Collines-de-l'Outaouais", "RLS du Pontiac")
