...
lapply(keys$cases, function(selection) {
    ggplot() +
        geom_line(data = covid[covid$key %in% selection[[1]] & covid$time > now() - days(42) & covid$table == "cases", ], mapping = aes(x = time, y = value, group = key, color = key)) + 
        geom_line(data = covid[covid$key %in% selection[[1]] & covid$time > now() - days(42) & covid$table == "areas", ], mapping = aes(x = time, y = value, group = key, color = key)) + 
        geom_line(data = covid[covid$key %in% selection[[1]] & covid$time > now() - days(42) & covid$table == "rls", ], mapping = aes(x = time, y = value, group = key, color = key)) + 
        labs(x = "", y = "") + 
        theme_classic() + 
...
## past year
```{r cases_year, echo = FALSE, results = 'hide', dpi = 225, fig.width = 7.5, fig.height = 5, out.width = "100%", out.height = "100%"}
lapply(keys$cases, function(selection) {
    ggplot() +
