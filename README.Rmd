...
rls <- c("RLS de Gatineau", "RLS de la Vallée-de-la-Gatineau", "RLS de Papineau", "RLS des Collines-de-l'Outaouais", "RLS du Pontiac")
keys <- list(
    cases = list(
        c("Cumulative cases", "Healed/resolved cases"),
        c("Active cases", "Deaths", "Average screening test per day")
    ),
    rls = list(
...
```{r figures, echo=FALSE, results='hide', fig.width = 7.5, fig.height = 5, out.width = "100%", out.height = "100%"}
lapply(keys$cases, function(keys) {
    list(ggplot(data = covid[covid$key %in% keys & covid$table == "cases", ]) + 
             geom_line(mapping = aes(x = time, y = value, group = key, color = key)) + 
             theme_classic() + labs(x = "", y = "") + 
             theme(legend.position = "bottom") + 
             ggtitle(paste(keys, collapse = " & ")),
         ggplot(data = covid[covid$key %in% keys & covid$table == "cases" & covid$time > now() - days(42), ]) + 
             geom_line(mapping = aes(x = time, y = value, group = key, color = key)) + 
             theme_classic() + labs(x = "", y = "") + 
             theme(legend.position = "bottom") + 
