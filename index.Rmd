...
In the figures of local data, "rugs" indicate dates for which the data are available in the dataset.
...
RegionFig <- function(df, second = NULL, bars = NULL,
                      caption, pop = NULL, dlabels = "%b %Y",
                      legend_cols = 3) {
  ggplot(data = df[df$date > "2020-03-15", ]) +
    { if(!is.null(bars)) 
      geom_bar(data = bars[bars$date > "2020-03-15", ], 
               mapping = aes(x = date, y = value),
               stat = "identity", color = "gray", alpha = 1/20)
    } + 
...
RegionFig(df = inspq[inspq$key == "Average increase per day", ], 
          bars = inspq[inspq$key == "New cases", ], pop = "per100k",
...
## Sources of covid-19 data for Outaouais & Quebec
