...
shows only cumulative snapshots by region and by [Réseaux locaux de services](https://www.inspq.qc.ca/covid-19/donnees/par-region) (RLS), though detailed, complete historical data are available from INSPQ with deeper digging.
The [COVID-19 Canada Open Data Working Group (opencovid.ca)](https://opencovid.ca/) provides official time series data at the provincial and health region levels.
However, none of these sources provide trend data below the regional level.
[CISSS Outaouais](https://cisss-outaouais.gouv.qc.ca/language/en/covid19-en/) provides frequent (sometimes daily) snapshots by RLS and by municipality, but again no trends.
The present project provides local trend data based on these snapshots and some key regional indicators.
...
RegionalFig <- function(df, caption, pop_perc = TRUE) {
  ggplot(data = df) +
    geom_line(mapping = aes(x = date, y = value, group = key, color = key)) +
    scale_x_date(date_breaks = "1 month", date_minor_breaks = "2 weeks", date_labels = "%d %b %Y") +
    { if(pop_perc) 
      scale_y_continuous(name = "", limits = c(0, NA),
                         sec.axis = sec_axis(trans = ~ . / pop_outaouais_2020 * 100, 
                                             name = "% of 2020 population"))
    } + 
    common_theme + 
    labs(x = "", y = "", caption = caption)
}
...
### Hospitalization
```{r hospitalization}
RegionalFig(df = hospitalization[hospitalization$key %in% c("Hospitalizations (non-ICU)", "Hospitalizations in ICU"), ],
            pop_perc = FALSE,
            caption = "Data source: www.donneesquebec.ca/recherche/dataset/covid-19-portrait-quotidien-des-hospitalisations")
```

### Vaccination
```{r vaccination}
RegionalFig(df = vaccination,
...
## Cumulative cases by municipalities and RLS {.tabset .tabset-fade .tabset-pills}
...
```{r rls_cumulative}
LocalFig(keys = c("RLS de la Vallée-de-la-Gatineau", "RLS de Papineau", 
...
## Available covid-19 data for Outaouais and Quebec

|                                                                       |                                                                                                                                                                    |
|-----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Centre intégré de santé et de services sociaux (CISSS) de l'Outaouais | https://cisss-outaouais.gouv.qc.ca/covid-19/                                                                                                                       |
...
| Institut national de santé publique du Québec (INSPQ)                 | https://www.inspq.qc.ca/covid-19/donnees                                                                                                                           |
...
| Ministère de la Santé et des services sociaux (MSSS)                  | https://www.donneesquebec.ca/recherche/dataset/covid-19-portrait-quotidien-des-hospitalisations                                                                    |
...
| COVID-19 Canada Open Data Working Group                               | https://opencovid.ca/api/                                                                                                                                          |
