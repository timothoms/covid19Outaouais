...
Grid lines indicate Mondays. If the figures below do not show the latest data, reload this page in your web browser.
...
LocalFig <- function(keys) {
  ggplot(data = covid[covid$key %in% keys & covid$table %in% c("areas", "rls"), ]) +
...
RLSdailyFig <- function(keys) {
  ggplot(data = daily[daily$change_key %in% keys & daily$table == "rls", ]) +
    ...
    # + ggtitle("Average increase per day")
}
RLSactiveFig <- function(keys) {
...
```{r cases_avg}
keys <- c("Average increase per day (Outaouais)")
ggplot(mapping = aes(x = date, y = daily_change_avg, group = change_key, color = change_key)) +
  geom_line(data = daily[daily$change_key %in% keys & daily$table == "cases", ]) +
...
```{r cases_cumulative}
keys <- c("Healed/resolved cases", "Total cases (Outaouais)") # "Total cases (RLS)"
...
```{r cases_other}
keys <- c("Active cases", "Average screening tests per day")
ggplot(mapping = aes(x = time, y = value, group = key, color = key)) +
  geom_line(data = covid[covid$key %in% keys & covid$time > now() - days(400) & covid$table == "cases", ]) +
  geom_line(data = covid[covid$key %in% keys & covid$time > now() - days(400) & covid$table == "areas", ]) +
  geom_line(data = covid[covid$key %in% keys & covid$time > now() - days(400) & covid$table == "rls", ]) +
...
## OpenCovid.ca & comparison to Ottawa {.tabset .tabset-fade .tabset-pills}

### Average increase

```{r opencovid_avg}
keys <- c("Average increase per day (Outaouais)", "Average increase per day (Ottawa)")
ggplot(mapping = aes(x = date, y = daily_change_avg, group = change_key, color = change_key)) +
...
```{r opencovid_total}
keys <- c("Total cases (Outaouais)", "Total cases (Ottawa)")
ggplot(mapping = aes(x = time, y = value, group = key, color = key)) +
...
### Deaths
```{r deaths_opencovid}
keys <- c("Total deaths (Outaouais)", "Total deaths (Ottawa)")
ggplot(mapping = aes(x = time, y = value, group = key, color = key)) +
  geom_line(data = covid[covid$key %in% keys & covid$time > now() - days(400) & covid$table == "opencovid.ca", ]) +
  scale_x_datetime(date_breaks="1 month", date_minor_breaks = "2 weeks", date_labels = "%b %Y") +
  lims(y = c(0, NA)) + labs(x = "", y = "") + common_theme
...
### Gatineau
```{r rls_new_gatineau}
RLSdailyFig(c("RLS de Gatineau"))
```

### Collines-de-l'Outaouais
```{r rls_new_collines}
RLSdailyFig(c("RLS des Collines-de-l'Outaouais"))
```

### Pontiac
```{r rls_new_pontiac}
RLSdailyFig(c("RLS du Pontiac"))
```

### Papineau
```{r rls_new_papineau}
RLSdailyFig(c("RLS de Papineau"))
```

### Vallée-de-la-Gatineau
```{r rls_new_vallee}
RLSdailyFig(c("RLS de la Vallée-de-la-Gatineau"))
```
...
The HTML source for the [CISSS Outaouais site](https://cisss-outaouais.gouv.qc.ca/language/en/covid19-en/) is downloaded daily, the tables are scraped and processed into a tidy dataset, to produce the figures on this site. The latest [opencovid.ca](https://opencovid.ca/) health region data are included for comparison. The source code for downloading and processing all data is available in the project [GitHub repository](https://github.com/timothoms/covid19Outaouais). Daily automation is currently done with MacScheduler.
