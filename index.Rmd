...
*April 3, 2021: Newly implemented layout changes to the CISSS website led to errors in yesterday's data update. These errors have now been fixed. Moreover, I have temporarily removed the hospitalization figure because CISSS Outaouais no longer reports these data. I will add historical covid19 hospitalization data provided by the province in the next few days.*
...
<!-- ### Cases in hospitals -->
```{r hospitals, eval = FALSE}
LocalFig(c("Hospitalizations", "Hospitalizations in ICU"), tab = "cases")
```
...
### Average increase
```{r opencovid_avg}
DailyFig(c("Average increase per day (Outaouais)", "Average increase per day (Ottawa)"), tab = "opencovid.ca", rug = FALSE)
```

### Cumulative cases
```{r opencovid_total}
LocalFig(c("Total cases (Outaouais)", "Total cases (Ottawa)"), tab = c("opencovid.ca"), rug = FALSE)
```

### Deaths
```{r deaths_opencovid}
LocalFig(c("Total deaths (Outaouais)", "Total deaths (Ottawa)"), tab = c("opencovid.ca"), rug = FALSE)
```
