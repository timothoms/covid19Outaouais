...
# vaccination$value / pop_outaouais_2020 * 100
...
```{r figure_functions}
LocalFig <- function(keys, tab = c("areas", "rls")) {
...
DailyFig <- function(keys, tab = "rls") {
  ggplot(data = daily[daily$change_key %in% keys & daily$table == tab, ]) +
    ...
    # geom_rug(mapping = aes(x = date), sides = "b") +
...
### Cases in hospitals
```{r hospitals}
LocalFig(c("Hospitalizations", "Hospitalizations in ICU"), tab = "hospital")
```
...
### Average increase
```{r opencovid_avg}
DailyFig(c("Average increase per day (Outaouais)", "Average increase per day (Ottawa)"), tab = "opencovid.ca")
```

### Cumulative cases
```{r opencovid_total}
LocalFig(c("Total cases (Outaouais)", "Total cases (Ottawa)"), tab = c("opencovid.ca"))
```

### Deaths
```{r deaths_opencovid}
LocalFig(c("Total deaths (Outaouais)", "Total deaths (Ottawa)"), tab = c("opencovid.ca"))
```
