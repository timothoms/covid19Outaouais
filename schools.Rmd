...
schools$school <- str_replace(schools$school, "École", "")
schools$school <- str_replace(schools$school, "School", "")
schools$school <- str_replace(schools$school, "Shcool", "")
...
SchoolsFig <- function(admin) {
  ggplot(data = schools[schools$admin %in% admin, ],
         aes(x = time, y = school)) +
    geom_point(mapping = aes(x = time, y = school, group = note, colour = note), size = 0.8) +
...
# Schools with confirmed covid-19 cases by Outaouais School Board {.tabset .tabset-fade .tabset-pills}
